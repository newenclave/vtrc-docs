...
Еще замечание: Клиент в любой момент времени можно отключить ```disconnect( );``` и подключить снова. Либо просто переподключить методом ```connect/async_connect```. 
...
Устанавливают id и ключ для соединения с сервером. Можно установить только ключ или только идентификатор. На стороне сервера ID клиента станет известен во время инициализации соединения. Ключ используется для настройки шифрования и если он не совпадет с тем, что выдаст клиенту сервер (см. описание ```server::application::get_session_key``` выше), соединение будет невозможною. Сервер такое соединение скинет, а клиент получит сигнал ```on_init_error```.
