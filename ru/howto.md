...
> Тут под протоколом понимается не низкий уровень, который бегает по каналу (о нем опишу отдельно), но протокол уровня сообщений и сервисов, которыми нужно оперировать при работе с библиотекой
...
Клиент и сервер могут находиться как на одном хосте, так и на разных континентах. Код будет выглядеть одинаково.
...
> Та самая часть библиотеки, которая используется и клиентом, и сервером. Тут я опишу некоторые общие сущности, которыми нужно оперировать.
